...
class AdapterRecyclerXML : RecyclerView.Adapter<AdapterRecyclerXML.HolderForeXML>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HolderForeXML {
        val binding = ElementRecyclerCommentsBinding.inflate(
    ...
    inner class HolderForeXML(
        private val binding: ElementRecyclerCommentsBinding
        ...
        fun bind(item: DataCard, position: Int) {
            ...
            binding.rightContainer.setOnClickListener { clickDeleteCurrent.invoke(item.id) }
            binding.columText.setOnClickListener { clickItemCurrent.invoke(item.id) }
        }
    }
}